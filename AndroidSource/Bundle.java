...
public final class Bundle extends BaseBundle implements Cloneable, Parcelable {
    ...
    public static void addBundleTaint(Bundle bundle, PathTaint parcelTaint, Object object) {
        ArrayList<String> extra = bundle.getStringArrayList("ZZZ_taintField");
        if (extra == null) {
            extra = new ArrayList<>();
        }
        String taint = PathTaint.parcelTaint(parcelTaint, object);
        // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
        // System.out.format("PathTaint: addBundleTaint: in method %s->%s, taint is %s %n", ste.getClassName(), ste.getMethodName(), taint);
        extra.add(taint);
        if (taint != null && !taint.isEmpty()) {
            bundle.putStringArrayList("ZZZ_taintField", extra);
        }
    }

    public static void addBundleTaint(Bundle bundle, PathTaint parcelTaint) {
        ArrayList<String> extra = bundle.getStringArrayList("ZZZ_taintField");
        if (extra == null) {
            extra = new ArrayList<>();
        }
        String taint = PathTaint.parcelTaint(parcelTaint);
        // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
        // System.out.format("PathTaint: addBundleTaint: in method %s->%s, taint is %s %n", ste.getClassName(), ste.getMethodName(), taint);
        extra.add(taint);
        if (taint != null && !taint.isEmpty()) {
    ...
    public static PathTaint getBundleTaint(Bundle bundle) {
        ArrayList<String> taintArray = bundle.getStringArrayList("ZZZ_taintField");
        if (taintArray != null && !taintArray.isEmpty()) {
            String taint = String.join("-", taintArray);
            ...
            return contained;
            // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
            // System.out.format("PathTaint: getBundleTaint: in method %s->%s, taint is %s %n", ste.getClassName(), ste.getMethodName(), taint);
        } else {
            return null;
            // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
            // System.out.format("PathTaint: getBundleTaint: in method %s->%s, taint is null %n", ste.getClassName(), ste.getMethodName());
        }
    }

    public static void addBundleTaint(Bundle bundle, int parcelTaint, Object object) {
        ArrayList<Integer> extra = bundle.getIntegerArrayList("ZZZ_taintField");
        if (extra == null) {
            extra = new ArrayList<>();
        }
        int taint = parcelTaint | TaintDroid.extractTaint(object);
        // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
        // System.out.format("PathTaint: addBundleTaint: in method %s->%s, taint is %s %n", ste.getClassName(), ste.getMethodName(), taint);
        if (taint != 0) {
            extra.add(taint);
            bundle.putIntegerArrayList("ZZZ_taintField", extra);
        }
    }

    public static void addBundleTaint(Bundle bundle, int parcelTaint) {
        ArrayList<Integer> extra = bundle.getIntegerArrayList("ZZZ_taintField");
        if (extra == null) {
            extra = new ArrayList<>();
        }
        int taint = parcelTaint;
        // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
        // System.out.format("PathTaint: addBundleTaint: in method %s->%s, taint is %s %n", ste.getClassName(), ste.getMethodName(), taint);
        if (taint != 0) {
            extra.add(taint);
    ...
    public static int getBundleTaintTaintDroid(Bundle bundle) {
        ArrayList<Integer> taintArray = bundle.getIntegerArrayList("ZZZ_taintField");
        int taint = 0;
        if (taintArray != null && !taintArray.isEmpty()) {
            for (int i = 0; i < taintArray.size(); i++) {
                taint |= taintArray.get(i);
            }
            // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
            // System.out.format("PathTaint: getBundleTaint: in method %s->%s, taint is %s %n", ste.getClassName(), ste.getMethodName(), taint);
        }
