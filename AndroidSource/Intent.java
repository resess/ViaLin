...
public class Intent implements Parcelable, Cloneable {
   ...
   /**
    ...
    public static void addParcelTaint(Intent intent, PathTaint parcelTaint, Object object) {
        String extra = intent.getStringExtra("ZZZ_taintField");
        String taint = PathTaint.parcelTaint(parcelTaint, object);
        // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
        // System.out.format("PathTaint: addParcelTaint: in method %s->%s, taint is %s %n", ste.getClassName(), ste.getMethodName(), taint);
        ...
        if (newTaint != null && !newTaint.isEmpty()) {
            intent.putExtra("ZZZ_taintField", newTaint);
        }
    }

    public static void addParcelTaint(Intent intent, PathTaint parcelTaint) {
        String extra = intent.getStringExtra("ZZZ_taintField");
    ...
    public static PathTaint getParcelTaint(Intent intent) {
        String taint = intent.getStringExtra("ZZZ_taintField");
        if (taint != null && !taint.isEmpty()) {
            ...
            return contained;
            // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
            // System.out.format("PathTaint: getParcelTaint: in method %s->%s, taint is %s %n", ste.getClassName(), ste.getMethodName(), taint);
        } else {
            return null;
            // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
            // System.out.format("PathTaint: getParcelTaint: in method %s->%s, taint is null %n", ste.getClassName(), ste.getMethodName());
        }
    }

    // TaintDroid
    public static void addParcelTaint(Intent intent, int parcelTaint, Object object) {
        int extra = intent.getIntExtra("ZZZ_taintField", 0);
        int taint = extra | parcelTaint | TaintDroid.extractTaint(object);
        if (taint != 0) {
            intent.putExtra("ZZZ_taintField", taint);
        }
    }

    public static void addParcelTaint(Intent intent, int parcelTaint) {
        int extra = intent.getIntExtra("ZZZ_taintField", 0);
        int taint = extra | parcelTaint;
        if (taint != 0) {
            intent.putExtra("ZZZ_taintField", taint);
        }
    }

    public static int getParcelTaintTaintDroid(Intent intent) {
        return intent.getIntExtra("ZZZ_taintField", 0);
    }
}
