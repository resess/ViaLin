    ...
    extends OutputStream implements ObjectOutput, ObjectStreamConstants
{
    ...
    /** Khaled: Taint for output stream */
    public PathTaint outStreamPathTaint = null;
    public String[] pathTaintContainer = null;
    public int outStreamTaintDroid = 0;
    public int[] taintDroidContainer = null;
    ...
    public ObjectOutputStream(OutputStream out) throws IOException {
        ...
        // Khaled: Taint
        outStreamPathTaint = Thread.getParamTaint1(Thread.currentThread());
        outStreamTaintDroid = Thread.getParamTaintTaintDroid1int(Thread.currentThread());
    }
    ...
    public final void writeObject(Object obj) throws IOException {
        // Khaled: add taint
        PathTaint newTaint = null;
        try {
            PathTaint objectTaint = Thread.getParamTaint1(Thread.currentThread());
            pathTaintContainer = new String[1];
            pathTaintContainer[0] = "";
            PathTaint.addSerializedTaint(pathTaintContainer, objectTaint, obj);
            newTaint = PathTaint.getSerializedTaint(pathTaintContainer);
            if (newTaint != null) {
                outStreamPathTaint = PathTaint.propagateTwoArgs(outStreamPathTaint, newTaint, Thread.currentThread().taintSite, Thread.currentThread().taintDelta);
                // outStreamPathTaint.site = Thread.currentThread().taintSite;
                // outStreamPathTaint.delta = Thread.currentThread().taintDelta;
                // outStreamPathTaint.left = newTaint;
                // outStreamPathTaint.right = objectTaint;
                Thread.setTaintContainer(outStreamPathTaint);
            }
        } catch (Exception e) {
            // System.out.format("FATAL: PathTaint: writeObject error: %s%n", e.getMessage());
            ...
            //     fullStackTrace.append("\n");
            // }
            // System.out.format("PathTaint: writeObject stack trace: %s%n", fullStackTrace.toString());
        }

        try {
            if (newTaint == null) {
                int objectTaint = Thread.getParamTaintTaintDroid1int(Thread.currentThread());
                taintDroidContainer = new int[1];
                taintDroidContainer[0] = 0;
                TaintDroid.addSerializedTaint(taintDroidContainer, objectTaint, obj);
                outStreamTaintDroid = TaintDroid.getSerializedTaint(taintDroidContainer);
            }
        } catch (Exception e) {
            // System.out.format("FATAL: PathTaint: writeObject error: %s%n", e.getMessage());
            // StringBuilder fullStackTrace = new StringBuilder();
