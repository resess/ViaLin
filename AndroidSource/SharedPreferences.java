...
public interface SharedPreferences {
    ...
    public interface Editor {
        ...
        public static void addSharedPrefsTaintTaintDroid(Editor editor, String key, int fileTaint, Object object) {
            int taint = TaintDroid.fileTaint(fileTaint, object);
            if (taint != 0) {
                // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
                // System.out.format("PathTaint: addSharedPrefsTaint: in method %s->%s, taint is %s %n", ste.getClassName(), ste.getMethodName(), taint);
                editor.putInt(key+"ZZZ_taint", taint);
            }
        }

        public static void addSharedPrefsTaintTaintDroid(Editor editor, String key, int taint) {
            if (taint != 0) {
                // StackTraceElement ste = Thread.currentThread().getStackTrace()[3];
                // System.out.format("PathTaint: addSharedPrefsTaint: in method %s->%s, taint is %s %n", ste.getClassName(), ste.getMethodName(), taint);
                editor.putInt(key+"ZZZ_taint", taint);
    ...
    public static PathTaint getSharedPrefsTaint(SharedPreferences prefs, String key) {
        String taint = prefs.getString(key+"ZZZ_taint", null);
    ...
    public static PathTaint getSharedPrefsTaintAll(SharedPreferences prefs) {
        StringBuilder taint = new StringBuilder();
        for (Map.Entry<String, ?> entry : prefs.getAll().entrySet()) {
            if (entry.getKey().endsWith("ZZZ_taint")) {
                if (entry.getValue() instanceof String) {
                    taint.append((String) entry.getValue());
                }
            }
        }
        String taintStr = taint.toString();
        if (taintStr != null && !taintStr.isEmpty()) {
            PathTaint contained = new PathTaint();
    ...
    public static int getSharedPrefsTaintTaintDroid(SharedPreferences prefs, String key) {
        int taint = prefs.getInt(key+"ZZZ_taint", 0);
        return taint;
    }

    public static int getSharedPrefsTaintAllTaintDroid(SharedPreferences prefs) {
        int taint = 0;
        for (Map.Entry<String, ?> entry : prefs.getAll().entrySet()) {
            if (entry.getKey().endsWith("ZZZ_taint")) {
                if (entry.getValue() instanceof Integer) {
                    taint |= (Integer) entry.getValue();
                }
            }
        }
        return taint;
