...
public final class Formatter implements Closeable, Flushable {
    ...
    /** Khaled: Taint for appendable */
    private PathTaint appendablePathTaint = null;
    private int appendablePathTaintTaintDroid = 0;
    ...
    public Formatter(Appendable a) {
        ...
        // Khaled: Taint
        appendablePathTaint = Thread.getParamTaint1(Thread.currentThread());
        appendablePathTaintTaintDroid = Thread.getParamTaintTaintDroid1int(Thread.currentThread());
    }
    ...
    public void addTaint() {
        ...
        try {
            ...
            if (objectTaint != null) {
                ...
                //     fullStackTrace.append("\n");
                // }
                // if (!fullStackTrace.toString().contains("java.lang.PathTaint")) {
                // System.out.println("Format called from: " + fullStackTrace.toString());
                if (appendablePathTaint == null) {
                    appendablePathTaint = PathTaint.propagateOneArg(objectTaint, Thread.currentThread().taintSite, Thread.currentThread().taintDelta);
                    // appendablePathTaint.site = Thread.currentThread().taintSite;
                    // appendablePathTaint.delta = Thread.currentThread().taintDelta;
                    // appendablePathTaint.left = objectTaint;
                } else {
                    appendablePathTaint = PathTaint.propagateTwoArgs(appendablePathTaint, objectTaint, Thread.currentThread().taintSite, Thread.currentThread().taintDelta);
                    // PathTaint newTaint1 = new PathTaint();
                    // newTaint1.site = appendablePathTaint.site;
                    // newTaint1.delta = appendablePathTaint.delta;
                    // newTaint1.left = appendablePathTaint.left;
                    // newTaint1.right = appendablePathTaint.right;

                    // appendablePathTaint.site = Thread.currentThread().taintSite;
                    // appendablePathTaint.delta = Thread.currentThread().taintDelta;
                    // appendablePathTaint.left = newTaint1;
                    // appendablePathTaint.right = objectTaint;
                }
                Thread.setTaintContainer(appendablePathTaint);
    ...
    public void addTaintTaintDroid() {
        // Khaled: add taint
        try {
            ...
            if (objectTaint != 0) {
                ...
                // if (!fullStackTrace.toString().contains("java.lang.PathTaint")) {
                // System.out.println("Format called from: " + fullStackTrace.toString());
                appendablePathTaintTaintDroid |= objectTaint;
                // }
            }
        } catch (Exception e) {
            e.printStackTrace();
